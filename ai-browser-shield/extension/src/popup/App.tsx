...
const TABS: { id: Tab; emoji: string; label: string }[] = [
  { id: 'score', emoji: 'Shield', label: 'Shield' },
  { id: 'email', emoji: 'Email', label: 'Email' },
  { id: 'history', emoji: 'History', label: 'History' },
  { id: 'feed', emoji: 'Feed', label: 'Feed' },
  { id: 'report', emoji: 'Report', label: 'Report' },
]
...
function getRiskKey(score: number | null) {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (changes: Record<string, chrome.storage.StorageChange>, areaName: string) => {
      ...
      if (
        ...
        changes.lastEmail ||
        changes.currentEmailAnalysis ||
        changes.lastEmailSnapshot ||
  ...
  const todayThreats = history.filter((event) => Date.now() - event.timestamp < 86400000).length
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          inset: 0,
          opacity: 0.03,
          backgroundImage: 'radial-gradient(circle at 20% 50%, #3B82F6 0%, transparent 50%), radial-gradient(circle at 80% 20%, #14B8A6 0%, transparent 50%)',
          pointerEvents: 'none',
        }} />
      ...
      <div style={{
        ...
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              <span style={{ fontSize: '10px', fontWeight: 800, lineHeight: 1 }}>{tab.emoji}</span>
              <span style={{
                fontSize: '10px',
                fontWeight: 700,
                letterSpacing: '0.3px',
                color: isActive ? '#5EEAD4' : '#334155',
      ...
      <div style={{ flex: 1, overflowY: 'auto', background: '#080C14' }}>
        ...
        {activeTab === 'email' && (
          <EmailGuard analysis={emailAnalysis} snapshot={emailSnapshot} isProcessing={emailProcessing} />
        )}
        {activeTab === 'history' && (
