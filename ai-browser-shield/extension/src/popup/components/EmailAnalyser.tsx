...
function getScoreColor(score: number): string {
  if (score >= 61) return '#EF4444' // Danger (Red)
  if (score >= 26) return '#EAB308' // Sus (Yellow)
...
export function EmailAnalyser() {
  ...
  const hasResult = emailStatus === 'ready' || emailStatus === 'deep_scanning' || filteredSignals.length > 0 || !!emailExplanation
  ...
  return (
    <div style={{ padding: '16px', color: '#E2E8F0' }}>
      ...
      <div style={{
        ...
        {(emailRiskScore >= 26 || emailStatus === 'deep_scanning') && (
          <div style={{ marginTop: '16px' }}>
            <button
              onClick={() => {
                chrome.runtime.sendMessage({
                  type: 'DEEP_SCAN_EMAIL',
                  payload: {
                    emailText,
                    senderEmail: '',
                    localSignals: filteredSignals.map(s => s.name)
                  }
                })
              }}
              disabled={emailStatus === 'deep_scanning'}
              style={{
                width: '100%',
                background: emailStatus === 'deep_scanning' ? '#1E293B' : 'linear-gradient(45deg, #1E3A5F, #0F172A)',
                border: '1px solid #3B82F6',
                color: '#93C5FD',
                borderRadius: '8px',
                padding: '10px',
                fontSize: '12px',
                fontWeight: 700,
                cursor: emailStatus === 'deep_scanning' ? 'wait' : 'pointer'
              }}
            >
              {emailStatus === 'deep_scanning' ? '🧬 Analyzing with Gemini AI...' : '🔍 Perform Deep AI Analysis'}
            </button>
            <div style={{ fontSize: '9px', color: '#64748B', textAlign: 'center', marginTop: '6px' }}>
              Recommended for suspicious or high-risk emails
            </div>
          </div>
        )}
      </div>
